...
- 이용법은 다양하게 있지만 대표적인 방법 2가지는 다음과 같다.
    1. ```c# float randomValue = Random.Value;  // Random.Value returns randomized value from 0 to 1 (both inclusive) ```
    2. ``` float randomValue = Random.Range(a, b); // Random.Range(a, b) returns randomized value from a to b (both inclusive) ```
