...
### std::vector
vector is a class template which handles the **variable-length** collection of objects of which have the same type
```c++
#include <vector>
...
|`<, <=, >, >=`|Comparisions have their normal meanings and use **dictioary ordering**|
```c++
string s1, s2;
