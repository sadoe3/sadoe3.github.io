...
- 얇은 렌즈 방정식은 주어진 초점 거리($f$)에 대해 이미지 거리($v$)가 초점 거리($u$)에 의해 고유하게 결정됨을 보여줍니다.
    ...
    <img src="/assets/images/raytracing/post3/choosing_image_plane.png" alt="choosing_image_plane" style="width: 70%; display: block; margin: 0 auto;">
...
- DoF 내의 픽셀은 거의 또는 전혀 흐림이 적용되지 않는 반면, 그 밖의 픽셀은 초점면에서의 거리에 따라 흐려져 디포커스 효과를 시뮬레이션합니다.
    ...
    <img src="/assets/images/raytracing/post3/defocus_blur.png" alt="defocus_blur" style="width: 70%; display: block; margin: 0 auto;">
...
- 픽셀의 최종 색상은 시간의 다른 순간에서의 색상의 혼합이며, 움직임의 착시를 만듭니다.
    ...
    <img src="/assets/images/raytracing/post3/motion_blur.png" alt="motion_blur" style="width: 70%; display: block; margin: 0 auto;">
- <details markdown="1">
...
* **성공 조건:** 광선이 사각형을 맞추는 경우는 $\mathbf{t \ge 0}$ 및 $\mathbf{0 \le \alpha \le 1}$ 및 $\mathbf{0 \le \beta \le 1}$일 때입니다.
    ...
    <img src="/assets/images/raytracing/post3/intersection.png" alt="intersection" style="width: 70%; display: block; margin: 0 auto;">
...
- 이 "**전부 아니면 전무(all-or-nothing)**" 접근 방식은 세부 정보 손실로 이어져 들쭉날쭉한 계단 모양을 유발합니다.
   ...
   <img src="/assets/images/raytracing/post3/aliasing.png" alt="aliasing" style="width: 70%; display: block; margin: 0 auto;">
...
- 널리 사용되는 두 가지 오퍼레이터는 다음과 같습니다:
    ...
    * **ACES (Academy Color Encoding System):** 특히 하이라이트에서 생생한 모양과 뛰어난 색상 채도 보존으로 알려진 보다 정교한 필름 룩(filmic) 톤 매핑 표준입니다.
    ...
    <img src="/assets/images/raytracing/post3/tonemapping.png" alt="tonemapping" style="width: 70%; display: block; margin: 0 auto;">
...
- 톤 매핑 후, 이미지는 특정 예술적 스타일을 적용하거나 색상 불균형을 수정하기 위해 **색상 변환(Color Transformation)**을 거칩니다. 이는 종종 **룩업 테이블(Look-Up Table, LUT)**을 사용하여 수행됩니다.
    ...
    * **효율성:** LUT를 통해 복잡한 색상 보정 작업(예: 대비 또는 화이트 밸런스 변경)을 적용하는 것은 픽셀당 단일 텍스처 조회 작업만 포함하므로, 모든 픽셀에 대해 긴 수학적 함수를 실행하는 것보다 훨씬 빠릅니다.
    ...
    <img src="/assets/images/raytracing/post3/lut.png" alt="lut" style="width: 70%; display: block; margin: 0 auto;">
