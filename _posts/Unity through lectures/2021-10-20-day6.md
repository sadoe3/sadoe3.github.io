...
- Animation clips 파일의 애니메이션을 볼 때에는 해당 animation 파일을 선택하면, inspector window에서 볼 수 있다.
    - 만약에 animation preview가 insepctor window에서 사라지게 되면, inspecto window의 우측 하단에 있는 점 3개 있는 버튼을 눌러 restore in Inspector를 선택하면 다시 animation preview를 불러올 수 있다.
