...
4. `cd` to the directory which you just `cloned`
5. Register the **nickname** `origin` to make it easier to use the **remote repository URL** in the terminal by using this code:
...
6. Set up the **tracking relationship** between your `local branch`(main) and the corresponding `remote branch` on origin(the remote repository) by using this command
    - `git push --set-upstream origin main`
7. `add` and `commit` the changes properly
8. Push changes to GitHub with `git push origin` and pull updates with `git pull origin`.
...
## Remove the Large File from the Git History
GitHub **restricts** the `size of individual` files to `100 MB`, and any file exceeding that limit **cannot be pushed** to a repository.

### How to Resolve
If you get an error message like `remote: error: File Folder/filename.exe is 148.44 MB; this exceeds GitHub's file size limit of 100.00 MB`, you can handle this issue by following the steps below
1. Add the file to `.gitignore` (if it's not already there):
2. Remove the file from Git's history
    * If you're on Git 2.23 or later, you can install `git-filter-repo` and use it to rewrite history more efficiently.
        ```js
        // install it
        pip install git-filter-repo
        ```
    * After the installation, run the command to remove the history
        ```js
        // remove the files or path
        // remove .vs/ path in this case
        git filter-repo --path .vs/ --invert-paths
        // if you don't want to backup the repo add --force command
        ```
3. Force push the changes to the remote repository
    * After removing the files from the history, you'll need to force-push the changes to the remote repository.
    * Be cautious with this step because force-pushing will overwrite history on the remote repository, which could cause issues for collaborators.
    * Make sure everyone is aware of this change.
        ```js
        git push --force --all
        ```
    * This will rewrite the history on all branches.
4. Prune and clean up your repository
    * After rewriting the history, you can run the following command to clean up the Git repository and remove any leftover references to the deleted files.
        ```js
        git gc --prune=now
        ```
...
**/.vs/
```
- Since `.vs/` is typically generated by `Visual Studio` and contains temporary or configuration data,
- it is usually a good idea to **ignore** this entire directory in your repository.

### Use Git Large File Storage (Git LFS)
If you need to track large files in the future, Git Large File Storage (`Git LFS`) is a better approach.
- Git LFS allows you to manage large files separately from your Git repository, storing them in an external storage service while keeping your Git repository size manageable.
- Install Git LFS:
    * macOS: `brew install git-lfs`
    * Windows: `choco install git-lfs` (if using Chocolatey)
    * Linux: Follow the installation instructions for your distribution.
- Initialize Git LFS:
    * `git lfs install`
- Track the large file(s):
    * To track your large files with Git LFS, use the following command to specify the file types you want to track:
        + `git lfs track "folder/file.exe"`
    * This will add the file to the .gitattributes file.
- Commit and push changes:
    * Now, `add` and `commit` the .gitattributes file and the large file:
        ```js
        git add .gitattributes
        git add folder/file.exe
        git commit -m "Track large file with Git LFS"
        git push origin main
        ```
- Git LFS will upload the large file to its storage service, and your repository will store a reference to the file instead of the file itself.
