...
5. local pc에서 cloning된 repository의 위치에, step1에서 만든 Unity Project 폴더 안에 있는 모든 파일들을 넣어준다.
    - 이때, 주의할 점은 Unity Project 폴더 통채로 넣는게 아니라, 폴더 안에 있는 파일들을 넣어줘야 한다.
        - 이렇게 해야, .gitignore 파일이 Unity Project 폴더 안에 존재하게 되어서, 정상적으로 작동하게됨
    - 폴더를 통채로 넣을거면, .gitignore파일을 해당 Unity Project 폴더 안에 넣어줘야, .gitignore 파일이 정상적으로 작동한다.  
6. 이후, local pc에서 Unity Project 폴더 안에 있는 .gitignore파일을 열어서, 위쪽부분에 있는 Libary, Temp, Build 폴더 등의 주석을 해제하여, 해당 폴더들을 git에서 ignore할 수 있도록 해준다.
