...
- inspector window에서 다양한 설정을 해줄 수 있음
    - ex: wrapping을 disabled 시키면, object 크기에 비해 font의 크기가 커도 그냥 무시하고 보여줌
...
- Rect Transform inspector의 왼쪽 상단에 있는 anchor preset을 이용하면, 쉽게 오브젝트의 위치를 지정할 수 있다.
    - anchor preset은 canvas object의 direct child일 때만 나온다.
...
- 이러한 singleton pattern을 사용하면, 해당 instance는 scene을 바꿔도 지워지않고 계속 유지된다.

## Size of camera object
- 카메라를 통해 화면보이는 정도를 좀더 키우고 싶다면, 즉 좀더 넓은 화면을 보여주고 싶다면, main camera object의 size값을 inspector에서 키워주면 된다.
- virtual camera를 사용하는 경우, main camera의 size가 아니라(어차피 안바뀜) virtual camera의 lens안에 있는 size값을 inspector에서 키워주면 된다.
