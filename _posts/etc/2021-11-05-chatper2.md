...
- Git에서 파일을 열때, 특정 VS Code로 파일을 수정하고 싶은경우 다음과 같은 방법을 사용하면 된다.
    ...
    2. 이렇게 되면, 특정한 파일을 열때 VS Code가 열리게 되고, 해당 파일을 저장하고 VS Code를 종료할때까지 cmd에서 command를 입력할 수 없게된다
...
## Git을 사용하는 방법
0. 위에서 정리한 내용들은 Git을 사용하기 위해 알고 있으면 좋은 배경지식들이고, 이번에 정리하는 내용이 사실 Git을 실질적으로 사용하는데 중요한 개념들이며, 다음과 같은 방법으로 Git을 local pc에서 사용할 수 있다.
