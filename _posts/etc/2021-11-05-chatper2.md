...
- local pc에서 project를 git을 기반으로 작업하게 되면, 다음과 같은 Workflow를 거치게 된다.
    - Git은 3가지의 위치에서 파일들을 관리한다: working directory, staging area, .git directory
...
## Git을 사용하는 방법
- 위에서 정리한 내용들은 Git을 사용하기 위해 알고 있으면 좋은 배경지식들이고, 이번에 정리하는 내용이 사실 Git을 실질적으로 사용하는데 중요한 개념들이다.
- Git은 다음과 같은 방법으로 사용할 수 있다.
...
- 이 방법은 [이곳](https://uang.tistory.com/11)을 참조하여 만들어졌습니다. 
...
- merge conflict를 해결하는 방법은 [이곳](https://www.youtube.com/watch?v=__cR7uPBOIk
)을 참조했습니다.
